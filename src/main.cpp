...
void updatePIDs(void* param) {
  ...
  while(true){
    ...
    lcd::print(4, (string("Lift Goal: ") + std::to_string(r->lift.getPIDGoal())).c_str());
    lcd::print(5, (string("Intake Goal: ") + std::to_string(r->intake.getPIDGoal())).c_str());
...
//Task odometryCalculations(calculatePosBASE, &rob.base.odom, "");
  ...
  while (true) {
    ...
    // base
    rob.base.driveArcade(master.rightY, master.leftX);
    if (master.rightY != 0 || master.leftY != 0) {
